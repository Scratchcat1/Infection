...
from tkinter import *
from multiprocessing import Process,Array,Pool,cpu_count,Queue,Value
...
class Machine():
    ...
    def Infect_Starter(self):
        ...
        for fdsa in range(self.cores):
            p = multiprocessing.Process(target = self.Infect(self.split_list[fdsa],self.split_list[fdsa+1]))
            procs.append(p)
            p.start()
        for p in procs:
            p.join()
...
##            self.Pygame_Enable = False
    ...
    def Infect(self,start,end):
        #print(start,":",end)
        for person in self.Population[start:end]:
