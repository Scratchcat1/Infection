...
from tkinter import *
from multiprocessing import Process,cpu_count,Pipe#,Array
...
##        self.cores = 2
    ...
    def Infect_Starter(self):
        ...
        for fdsa in range(self.cores):
            ...
            p = multiprocessing.Process(target = self.Infect(self.split_list[fdsa],self.split_list[fdsa+1],Child_conn))
            print("hi2")
            procs.append([p,Parent,Child_conn])
            p.start()
        self.Population =[]

        for p in procs:
            self.Population +=p[1].recv()
            p[0].join()
...
##            self.Pygame_Enable = False
    ...
    def Infect(self,start,end,Child_conn):
        #print(start,":",end)
        ...
        Child_conn.send(Population[start:end])
        Child_conn.close()
