...
class Machine():
    ...
    ##    Use_File = input("Open file? [Yes or No]").lower()
        ...
        self.Population[Infected]._Infected = True
        self.day =1
    ...
    def Set_Grid(self):
        ...
        if self.Population != []:
            for person in self.Population:
                person._x,person._y= self.XCurrent+self.radius,self.YCurrent+self.radius
...
##                    break
            ...
            for asdf in range(int(Vac_Val)):
                ...
                self.Population[num]._Vaccinating = True
    ...
    def Add_Population(self):
        ...
        for ghj in range(len(self.Population),self.Population_Size):
    ...
    def Days(self,Autodays,Pygame_Enable):
        for tempoaryautodays in range(wait_time):
            ...
            self.Population = Update_Status(self.Population)
            if Pygame_Enable:
                gameDisplay.fill(white)

                
            ## Has to be seperate to avoid people coming in contact multiplte times        
            self.Population = Infect(self.Population,self.infection_chance)
